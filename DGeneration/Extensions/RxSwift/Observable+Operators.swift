...
extension ObservableType {
    ...
    func mapToBool(_ value: Bool) -> Observable<Bool> {
        return map { _ in value }
    }
}

let kVHRefreshOffset: CGFloat = -100
...
extension Reactive where Base: UIScrollView {
    ...
    public var pullOffsetY: Observable<CGFloat> {
        let scrollView = self.base as UIScrollView
        return self
            .contentOffset
            .map { $0.y }
            .flatMap { [weak scrollView] contentOffsetY -> Observable<CGFloat> in
                guard let scrollView = scrollView else { return Observable.empty() }
                var realOffsetY = contentOffsetY
                if #available(iOS 11.0, *) {
                    realOffsetY += max(scrollView.safeAreaInsets.top, scrollView.contentInset.top)
                } else {
                    realOffsetY += scrollView.contentInset.top
                }
                return Observable.just(realOffsetY)
            }
    }
    
    func pullPercent(distance: CGFloat) -> Observable<CGFloat> {
        self.pullOffsetY.map { min(max($0 / distance, 0), 1) }
    }
    
    public var pullRefresh: Observable<Void> {
        let scrollView = self.base as UIScrollView
        return self
            .didEndDragging
            .withLatestFrom(self.contentOffset)
            .flatMap { [weak scrollView] contentOffset -> Observable<Void> in
                guard let scrollView = scrollView else { return Observable.empty() }
                var realOffsetY = contentOffset.y
                if #available(iOS 11.0, *) {
                    realOffsetY += max(scrollView.safeAreaInsets.top, scrollView.contentInset.top)
                } else {
                    realOffsetY += scrollView.contentInset.top
                }
                return realOffsetY < kVHRefreshOffset ? Observable.just(()) : Observable.empty()
            }
    }
}
